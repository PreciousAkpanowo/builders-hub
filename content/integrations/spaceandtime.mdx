...
## Overview
Space and Time is a decentralized database that comes preloaded with indexed blockchain data from major blockchains including Bitcoin, Ethereum, Avalanche, Arbitrum, Polygon, Base, Sui, etc. Developers can use Space and Time to access comprehensive onchain data, index their smart contract events, query the data with SQL, join it with their offchain datasets, and ZK-prove the result.
